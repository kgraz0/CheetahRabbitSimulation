...
The game simulates two automated players, Cheetah and Rabbit, on a 2D grid where they are both able to move Up, Down, Left or Right. There is a border set to make sure that either of them do not go out of bounds. They both move in random directions until the Cheetah eventually catches the Rabbit by both ending up on the same tile. If the user decides to select the second option in the menu, they are able to specify the length of walk (amount of moves) and the spawning location (X and Y) of the cheetah. In this mode, the movement of Cheetah is recorded and saved into the SavedWalks.txt file.